function wellMapper() {
    ...
    this.initialize = function ( Target ) {
       //var sources = [{ id: 1, type: 'blood', position: 'A1'}, { id : 2, type : 'blood', position : 'A2'}];
        ...
        this.x_max = Target.max_row || rows[rows.length-1];
        this.y_min = Target.min_col || 1 || cols[0];
        this.y_max = Target.max_col || cols[cols.length-1];
    ...
    this.distribute = function ( sources, Target ) {
        ...
        console.log("Options: " + JSON.stringify(Target));
        ...
        if ( ! this.pack ) {
            ...
            for (var x=1; x<=repeat_set; x++) {  // only repeats when split in parallel mode
...
describe('wellMapper()', function() {
    ...
    describe('* distribute', function () {
        ...
        var Target = { max_row : 'C', max_col : 2 };
    
        var sources = [ { id: 1 }, { id: 2} ];
        
        var complex_sources = [ { id: 3, position: 'A1'} , { id: 4, position: 'A2'} ]

        // target options:  transfer_qty, transfer_qty_units, format: N, sample_type: N, split: N, pack_wells: N, split_mode: serial/parallel,  


        it('simple 1 to 1 transfer', function () {

          assert.equal(6, map.testFunction(3));
        });

        it('split 1 -> 2 well transfer (parallel)', function () {

          assert.equal(4, map.testFunction(2));
        });

        it('split 1 -> 2 well transfer (serial)', function () {

          assert.equal(4, map.testFunction(2));
        });

        it('split 1 -> 2 well transfer (pack)', function () {
